...
#'
#' @description Builds pathways adjacency matrices from specified KEGG identifers.
...
#'
#' @examples keggid = list("Unk1", "C00350","C00641","Unk2","Unk3",c("C08363", "C00712"))
...
################### convertTable ####################
# MetaboSignal function : http://bioconductor.org/packages/release/bioc/html/MetaboSignal.html
...
FindMetabolitePathways = function(compoundM=NULL, organism_code=NULL){
  ...
  path.org = unique(convertTable(getURL(paste0("http://rest.kegg.jp/link/pathway/",organism_code)))[,2]) # check pathways in the organism
  compoundPath = compoundPath[compoundPath[,2]%in%path.org,]
  return(compoundPath)
}

#################### MembershipFromPathway ####################
MembershipFromPathways = function(compoundPath=NULL){
  compoundPath = compoundPath[order(compoundPath[,"KEGG compound"]),]
...
GetGraphPathway = function(pathId){
  ...
  pathkgml = parseKGML(getURL(file))
  reactions = suppressWarnings(KEGGpathway2reactionGraph(pathkgml))
  if(is.null(reactions)) return(NULL)
  inet = igraph.from.graphNEL(reactions)
  return(inet)
}
...
GetDistanceMat = function(membmat){
  ...
  inet.list = lapply(inet.list, function(x) {
    vname = which(!is.element(unique(paste0("cpd:",unlist(metset))), get.vertex.attribute(x,"name")))
    x = add.vertices(x, length(vname), attr = list(name=unique(paste0("cpd:",unlist(metset)))[vname]))})
  #get distance mat,
  dmat = lapply(inet.list, distances, mode="all")
  ...
  #deal with non unique kegg ids
  dmat_uq = lapply(dmat, function(x) as.matrix(aggregate(x, by=list(id=idx), FUN=min))[,-1]) #try mean or other functions as well
  dmat_uq = lapply(dmat_uq, function(x) {rownames(x) = rownames(membmat); x})
  dmat_uq = lapply(dmat_uq, function(x) as.matrix(aggregate(t(x), by=list(id=idx), FUN=min))[,-1])
  ...
  nn = sapply(nodup, function(i,x) paste(names(dmat_uq)[sapply(x, identical, dmat_uq[[i]])],collapse="|") ,x=dmat_uq)
  dmat_uq = dmat_uq[nodup]
  #remove distance matrices with elements that are all infinite
  nn = nn[sapply(dmat_uq, function(x) sum(1/x[lower.tri(x)])>0)]
  dmat_uq = dmat_uq[sapply(dmat_uq, function(x) sum(1/x[lower.tri(x)])>0)]
  names(dmat_uq) = nn
  return(dmat_uq)
}
