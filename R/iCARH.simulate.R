...
#' \item{arx}{autregressive coefficients for X data}
#' \item{ary}{autregressive coefficients for Y data}
#'
...
iCARH.simulate = function(Tp, #number of time points
  ...
  if(length(path.names)){
    ...
    compounds = abind(lapply(path.names, function(x){
      convertTable(getURL(paste0("http://rest.kegg.jp/link/cpd/", x)))}),along=1)
    ...
    if(is.logical(path.probs)){
      ...
      all.compounds = unique(convertTable(getURL("http://rest.kegg.jp/list/compound"))[,1])
      pathway.nb = sapply(all.compounds,function(x){
        pp = convertTable(getURL(paste0("http://rest.kegg.jp/link/pathway/", x)));
      ...
      A = A[,!duplicated(A, MARGIN = 2) & colSums(A)>1]
      P.sim = ncol(A)
      if(P.sim != rankMatrix(A)) warning("Model might not be identifiable.
  ...
  {if(y==1) c(rtrunc(distr = rnorm, 1, linf=0, lsup=1/(x[2])-0.005,mean=1/(x[2])-0.005, sd=noise.phi),
              rtrunc(distr = rnorm, 1, linf=1/(x[1])+0.005, lsup=0,mean=1/(x[1])+0.005, sd=noise.phi))
    else rep(rtrunc(distr = rnorm, 1, linf = 1/(x[1])+0.005, lsup = 1/(x[2])-0.005,
